...
function scheduleInactiveCleanup(activeGames, io) {
  ...
  console.log("Scheduled inactive game cleanup.");
...
export class GenericGameState {
  ...
  addCustomGameState(stateName) {
    ...
    this.#customGameState[stateName] = this.#customGameState.length + 100; // Start custom states after internal ones
  }
...
export class GenericGameLogic {
  ...
  initializeGameData(hostSocketId, clientCreateData) {
    throw new Error(`"initializeGamedata" not implemented in ${this.constructor.name}`);
  }
  getJoiningPlayerProps(currentPlayerss, clientJoinData) {
    throw new Error(`"getJoiningPlayerProps" not implemented in ${this.constructor.name}`);
  }
  isActionValid(game, playerMakingMove, actionData) {
    throw new Error(`"isActionValid" not implemented in ${this.constructor.name}`);
  }
  updateGameState(currentState, actionData, playerSymbol) {
    throw new Error(`"updateGameState" not implemented in ${this.constructor.name}`);
  }
  checkEndConditions(gameState, game) {
    throw new Error(`"checkEndConditions" not implemented in ${this.constructor.name}`);
  }
  postAction(currentState, lastPlayerId, winner, isDraw, game) {
    throw new Error(`"postAction" not implemented in ${this.constructor.name}`);
  }
  getResetState(game) {
    throw new Error(`"getResetState" not implemented in ${this.constructor.name}`);
  }
  ...
  registerEventHandlers() {
    ...
    this.#socket.on("hostInitiatedGameStart", this.#handleHostInitiatedGameStart.bind(this));
  ...
  #handleCreateGame(clientData) {
    ...
    this.#socket.emit("gameCreatedWaitingForPlayers", {
      pin,
      playerSymbol: gameInitConfig.initialPlayer.piece,
      playerName: name,
      playerList: activeGames[pin].players.map(p => ({ name: p.name, symbol: p.symbol, id: p.id })),
      gameType: this.gameType,
      minPlayers: activeGames[pin].minPlayers,
      maxPlayers: activeGames[pin].maxPlayers,
      hostId: this.#socket.id
    });
    console.log(`Lobby for game ${pin} (${this.gameType}) create by host ${name} (${this.#socket.id}). Waiting for players. Min: ${this.minPlayers}, max: ${this.maxPlayers}`);
  }
  ...
  #handleJoinGame(clientData) {
    ...
    if (game.players.find(p => p.id === this.#socket.id)) {
      console.log(`Player ${this.#socket.id} is already in the lobby for the game ${clientData.pin}.`);
      return;
    }
  ...
  #handleHostInitiatedGameStart(clientData) {
    if (!clientData.pin) {
